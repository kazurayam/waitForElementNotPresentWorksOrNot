...
class TL1 {
	...
	def afterTestSuite(TestSuiteContext testSuiteContext) {
		...
		executeWkhtmltopdf(reportFolder, html, pdf)
	}
	
	void executeWkhtmltopdf(Path reportFolder, String html, String pdf) {
		Subprocess.CompletedProcess cp;
		cp = new Subprocess().cwd(reportFolder.toFile())
				.run(Arrays.asList("/usr/local/bin/wkhtmltopdf", html, pdf, "--run-script", "expandAllChildren('s1')"));
